...
class ViewController: UIViewController {
    ...
    @IBAction func basicsTest(_ sender: Any) {
        ...
        let classObj = SampleClass(name: "Emin Teyhan", surName: "Uslu").setTitleObject(by: structObj)
        classObj.display()
        
        let computerStruct = ComputerStruct(OS: .MacOS, brand: "Apple", RAM: 16, memoryType: .SSD)
        let computerClass = ComputerClass(computer: computerStruct, buyFrom: "Apple Store")
        computerClass.display()
    }
